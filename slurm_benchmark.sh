...
do
    ...
    do
        ...
        do
            set -- $i
            sbatch -o "slurm/hendrixgpu${node}fl/${gpus}/matmul.out" -p gpu --ntasks=1 --cpus-per-task=8 --mem=8G --gres=gpu:${gpus} -w hendrixgpu${node}fl --time=2-00:00:00 ./benchmark_matmul.sh $REPEATS ${gpus} $1 $2 ${node}
        done
    done
done